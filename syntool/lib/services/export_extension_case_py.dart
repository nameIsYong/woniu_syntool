...
//糖尿高血压专案模块
extension ExportPyCaseExtension on ExportDataService {
  //导出高血压专案数据
  static Future<ImportDataResponse> httpExportPyCaseDatas({
    ...
    result = await httpExportNotSynPyCaseDatas(
      pageIndex: 1,
  ...
  //导出未同步的高血压专案数据
  static Future<ImportDataResponse> httpExportNotSynPyCaseDatas({
    ...
    try {
      ...
      print("请求地址（未同步的高血压专案数据）---->$apiPath,n请求参数---->$formBody");
      // 发送POST请求 - 使用表单格式提交数据
      ...
      if (jsonResponse['status'] == 0) {
        ...
        for (var item in results) {
          ...
          userData.moduleType = ModuleType.kCaseHyp;
          resultModel.importedDataList.add(userData);
          index++;
        }
        if (isHasNextPage) {
          // 获取下一页数据
          return await httpExportNotSynPyCaseDatas(
          ...
          //查询同步失败的
          return await httpExportFailSynPyCaseDatas(
            pageIndex: 1,
            token: token,
            resultModel: resultModel,
            areaCode: areaCode,
  ...
  //导出高血压专案同步失败的数据
  static Future<ImportDataResponse> httpExportFailSynPyCaseDatas({
    required int pageIndex,
    required String token,
    required ImportDataResponse resultModel,
    required String areaCode,
    ...
    try {
      ...
      var formBody = params.entries
          ...
          .join('&');
      var apiPath = 'http://bmg.2woniu.cn/bmanage/publichealth/chronic/list';
      print("请求地址（同步失败的高血压专案数据）---->$apiPath,n请求参数---->$formBody");
      // 发送POST请求 - 使用表单格式提交数据
      var response = await http.post(
        Uri.parse(apiPath),
        headers: headers,
