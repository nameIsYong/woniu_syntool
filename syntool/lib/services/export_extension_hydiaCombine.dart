...
//高糖合并随访模块
extension ExportHypDiaCombineExtension on ExportDataService {
  // 导出高糖合并随访数据
  static Future<ImportDataResponse> httpExportHypDiaCombineDatas({
    ...
    result = await ExportHypDiaCombineExtension.httpExportNotSynHyDiaCombineDatas(
      pageIndex: 1,
  ...
  // 导出未同步的高糖合并随访数据
  static Future<ImportDataResponse> httpExportNotSynHyDiaCombineDatas({
    ...
    try {
      ...
      var formBody = params.entries
          ...
          .join('&');
      var apiPath =
          'http://bmg.2woniu.cn/bmanage/publichealth/service/record/6/list';
      print("请求地址（未同步的高糖合并随访）---->$apiPath,n请求参数---->$formBody");
      // 发送GET请求 - 参数通过URL传递
      ...
      if (jsonResponse['status'] == 0) {
        ...
        for (var item in results) {
          ...
          userData.moduleType = ModuleType.kHSM;
          resultModel.importedDataList.add(userData);
          index++;
        }
        if (isHasNextPage) {
          // 获取下一页数据
          return await ExportHypDiaCombineExtension.httpExportNotSynHyDiaCombineDatas(
